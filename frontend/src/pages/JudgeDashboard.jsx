    ...
    <div class="space-y-6">
      ...
      <div class="glass-card p-6 sm:p-8 rounded-3xl relative overflow-hidden border border-slate-800 bg-gradient-to-r from-slate-900 via-indigo-950/20 to-slate-900">
        ...
        <div class="flex items-center space-x-4">
          ...
          <div>
            <h2 class="text-2xl font-bold text-white">Expert Evaluation Panel</h2>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="glass-card p-5 rounded-2xl border border-slate-800 flex justify-between items-center">
          <div>
            <span class="text-[10px] text-slate-500 uppercase tracking-wider font-semibold block">Total Projects</span>
            <span class="text-2xl font-extrabold text-white mt-1 block">{stats.total_submissions}</span>
          </div>
          <div class="h-10 w-10 rounded-lg bg-indigo-500/10 flex items-center justify-center text-indigo-400 border border-indigo-500/15">
        ...
        <div class="glass-card p-5 rounded-2xl border border-slate-800 flex justify-between items-center">
          <div>
            <span class="text-[10px] text-slate-500 uppercase tracking-wider font-semibold block">Evaluated</span>
            <span class="text-2xl font-extrabold text-emerald-400 mt-1 block">{stats.evaluated_submissions}</span>
          </div>
          <div class="h-10 w-10 rounded-lg bg-emerald-500/10 flex items-center justify-center text-emerald-400 border border-emerald-500/15">
        ...
        <div class="glass-card p-5 rounded-2xl border border-slate-800 flex justify-between items-center">
          <div>
            <span class="text-[10px] text-slate-500 uppercase tracking-wider font-semibold block">Pending</span>
            <span class="text-2xl font-extrabold text-amber-500 mt-1 block">{stats.pending_submissions}</span>
          </div>
          <div class="h-10 w-10 rounded-lg bg-amber-500/10 flex items-center justify-center text-amber-500 border border-amber-500/15">
      ...
      {/* Submissions List */}
      <div class="space-y-4">
        <h3 class="text-lg font-bold text-white">Projects Submitted</h3>
        
        {isLoading ? (
          <div class="py-12 flex flex-col items-center justify-center">
            <div class="h-8 w-8 border-2 border-indigo-500/20 border-t-indigo-500 rounded-full animate-spin"></div>
            <p class="text-xs text-slate-500 mt-2">Loading projects...</p>
          </div>
        ) : submissions.length === 0 ? (
          <div class="glass-card p-12 text-center text-slate-500 text-xs rounded-2xl border border-slate-800">
            No projects have been submitted to the portal yet.
          </div>
        ) : (
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            {submissions.map((sub) => (
              <div key={sub.id} class="glass-card p-6 rounded-2xl border border-slate-800 flex flex-col justify-between">
                <div class="space-y-3">
                  <div class="flex justify-between items-start">
                    <div>
                      <span class="text-[10px] text-indigo-400 font-semibold tracking-wide uppercase">{sub.hackathon_title}</span>
                      <h4 class="font-bold text-white text-base mt-0.5">{sub.project_title}</h4>
                      <p class="text-xs text-slate-500 mt-0.5">By Team: <span class="text-slate-300 font-semibold">{sub.team_name}</span></p>
                    </div>
                  ...
                  <p class="text-xs text-slate-400 line-clamp-3">{sub.description}</p>
                  ...
                  <div>
                    {sub.is_evaluated ? (
                      <div class="flex items-center space-x-1">
                        ...
                        <span class="text-sm font-bold text-indigo-400">{sub.scores.total}</span>
      ...
      {selectedSub && (
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div class="glass-card w-full max-w-lg p-6 sm:p-8 rounded-3xl border border-slate-800 shadow-2xl space-y-5 max-h-[90vh] overflow-y-auto">
            <div class="flex justify-between items-center">
              <div>
                <h3 class="text-lg font-bold text-white">Grade Project: {selectedSub.project_title}</h3>
                <p class="text-xs text-slate-500 mt-0.5">Team: {selectedSub.team_name} | {selectedSub.hackathon_title}</p>
              </div>
              <button 
                onClick={() => setSelectedSub(null)}
                class="text-slate-400 hover:text-slate-200 text-sm font-semibold cursor-pointer"
              >
                Cancel
              </button>
            </div>
            ...
            <form onSubmit={handleEvaluateSubmit} class="space-y-4">
              <div class="grid grid-cols-2 gap-4">
                ...
                <div class="space-y-1">
                  <label class="text-xs font-medium text-slate-300 flex justify-between">
                    ...
                    <span class="text-indigo-400 font-bold">{innovation}/25</span>
                  </label>
                  <input
                    type="range"
                    min="0"
                    ...
                    class="w-full h-1.5 bg-slate-900 border border-slate-800 rounded-lg appearance-none cursor-pointer accent-indigo-500"
                  />
                </div>
                ...
                <div class="space-y-1">
                  <label class="text-xs font-medium text-slate-300 flex justify-between">
                    ...
                    <span class="text-indigo-400 font-bold">{technical}/25</span>
                  </label>
                  <input
                    type="range"
                    min="0"
                ...
                <div class="space-y-1">
                  <label class="text-xs font-medium text-slate-300 flex justify-between">
                    ...
                    <span class="text-indigo-400 font-bold">{feasibility}/25</span>
                  </label>
                  <input
                    type="range"
                    min="0"
                ...
                <div class="space-y-1">
                  <label class="text-xs font-medium text-slate-300 flex justify-between">
                    ...
                    <span class="text-indigo-400 font-bold">{presentation}/25</span>
                  </label>
                  <input
                    type="range"
                    min="0"
              ...
              {/* Total Score display */}
              <div class="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800 flex justify-between items-center text-sm font-semibold">
                <span class="text-slate-400">Total Scorecard Value:</span>
                <span class="text-base text-indigo-400 font-extrabold">
              ...
              <div>
                <label for="eval_comments" class="block text-xs font-medium text-slate-300">
                  Judgement & Feedback
                </label>
                <textarea
                  id="eval_comments"
