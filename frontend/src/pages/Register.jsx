    ...
    <div class="min-h-screen bg-slate-950 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div class="sm:mx-auto sm:w-full sm:max-w-md z-10">
        <div class="flex justify-center">
          <Link to="/login" class="flex items-center text-xs font-semibold text-slate-500 hover:text-slate-300 transition-colors mb-4 cursor-pointer">
        ...
        <p class="mt-2 text-center text-sm text-slate-400">
          ...
          <Link to="/login" class="font-medium text-indigo-400 hover:text-indigo-300 transition-colors">
            Sign in
          </Link>
        </p>
      </div>
      ...
      <div class="mt-6 sm:mx-auto sm:w-full sm:max-w-md z-10 px-4">
        <div class="glass-card py-8 px-6 sm:px-10 rounded-3xl shadow-2xl border border-slate-800">
          {error && (
            <div class="mb-5 p-4 rounded-xl bg-rose-500/10 border border-rose-500/20 flex items-start space-x-2.5 text-rose-400 text-sm">
          ...
          {success && (
            <div class="mb-5 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-start space-x-2.5 text-emerald-400 text-sm">
              <AlertCircle class="h-5 w-5 shrink-0" />
              <span>{success}</span>
            </div>
          )}
          ...
          <form class="space-y-5" onSubmit={handleSubmit}>
              ...
              <div class="mt-2 grid grid-cols-2 gap-3">
                <button
                  ...
                  class={`py-2.5 text-sm font-semibold rounded-xl border transition-all cursor-pointer ${
                    role === 'participant'
                      ? 'bg-indigo-600/20 text-indigo-400 border-indigo-500 shadow-inner shadow-indigo-600/10'
            ...
            <div>
              ...
              <div class="mt-1.5 relative">
                ...
                <input
                  ...
                  class="block w-full pl-10 pr-4 py-3 bg-slate-900/60 border border-slate-800 rounded-xl text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all text-sm"
            ...
            {role === 'participant' && !showNewCollegeForm && (
              <div>
                <label for="college" class="block text-sm font-medium text-slate-300 flex justify-between">
                  <span>Select College</span>
                  <button
                    type="button"
                    onClick={() => setShowNewCollegeForm(true)}
                    class="text-xs text-indigo-400 hover:text-indigo-300 font-medium cursor-pointer"
                  >
                    Add new college?
                  </button>
                </label>
                <div class="mt-1.5 relative">
                  ...
                  <select
                    ...
                    required
                    class="block w-full pl-10 pr-4 py-3 bg-slate-900/60 border border-slate-800 rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all text-sm appearance-none"
                  >
                    <option value="" class="bg-slate-950">Select your college</option>
                    {colleges.map((col) => (
                      <option key={col.id} value={col.id} class="bg-slate-950">
                        {col.college_name}
                      </option>
                    ))}
                  </select>
            ...
            {showNewCollegeForm && (
              <div class="p-4 rounded-2xl bg-indigo-950/15 border border-indigo-500/15 space-y-4">
                <div class="flex justify-between items-center">
                  <h3 class="text-xs font-bold tracking-wider text-indigo-400 uppercase">
                ...
                <div>
                  <label for="college_name" class="block text-[11px] font-medium text-slate-400">
                    College Name *
                  </label>
                  <input
                    id="college_name"
                    ...
                    class="mt-1 block w-full px-3 py-2 bg-slate-900/50 border border-slate-800 rounded-lg text-slate-200 focus:outline-none focus:ring-1 focus:ring-indigo-500 text-xs"
                    placeholder="E.g., Stanford University"
                  />
                </div>

                <div>
                  <label for="college_address" class="block text-[11px] font-medium text-slate-400">
                    Address / Location
                  </label>
                  <input
                    id="college_address"
                ...
                <div>
                  <label for="college_contact" class="block text-[11px] font-medium text-slate-400">
                    Contact Person Name
                  </label>
                  <input
                    id="college_contact"
